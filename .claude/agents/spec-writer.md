...
name: spec-writer
description: >
  Turns a project brief into a specification an agent can implement without guessing -
  numbered acceptance criteria, an out-of-scope list, and the open questions still marked
  open. Use after the analyst has produced a brief. Do not use for implementation planning
  or for code.
